...
namespace Electricity.Utils {
    public class Selection {
        ...
        public BlockFacing Face {
            get {
                ...
                if (normalize.X > normalize.Y && normalize.X > normalize.Z && normalize.X > -normalize.Y && normalize.X > -normalize.Z)
                    return this.didOffset ? BlockFacing.WEST : BlockFacing.EAST;

                if (normalize.X < normalize.Y && normalize.X < normalize.Z && normalize.X < -normalize.Y && normalize.X < -normalize.Z)
                    return this.didOffset ? BlockFacing.EAST : BlockFacing.WEST;

                if (normalize.Z > normalize.Y && normalize.Z > normalize.X && normalize.Z > -normalize.Y && normalize.Z > -normalize.X)
                    return this.didOffset ? BlockFacing.NORTH : BlockFacing.SOUTH;

                if (normalize.Z < normalize.Y && normalize.Z < normalize.X && normalize.Z < -normalize.Y && normalize.Z < -normalize.X)
                    return this.didOffset ? BlockFacing.SOUTH : BlockFacing.NORTH;

                if (normalize.Y > normalize.X && normalize.Y > normalize.Z && normalize.Y > -normalize.X && normalize.Y > -normalize.Z)
                    return this.didOffset ? BlockFacing.DOWN : BlockFacing.UP;

                if (normalize.Y < normalize.X && normalize.Y < normalize.Z && normalize.Y < -normalize.X && normalize.Y < -normalize.Z)
                    return this.didOffset ? BlockFacing.UP : BlockFacing.DOWN;
        ...
        public Facing Facing {
            get => FacingHelper.From(this.Face, this.Direction);
        }
