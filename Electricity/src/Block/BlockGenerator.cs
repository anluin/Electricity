...
{
    public class BlockGenerator : Vintagestory.API.Common.Block, IMechanicalPowerBlock
    {
        ...
        {
            if (world.BlockAccessor.GetBlockEntity(pos) is BlockEntityGenerator entity && entity.Facing != Facing.None)
            ...
            BlockSelection blockSel, ref string failureCode)
        {
            ...
            if (FacingHelper.Faces(facing).First() is { } blockFacing
                && !world.BlockAccessor.GetBlock(blockSel.Position.AddCopy(blockFacing))
            ...
            ItemStack byItemStack)
        {
            ...
            if (base.DoPlaceBlock(world, byPlayer, blockSel, byItemStack)
                && world.BlockAccessor.GetBlockEntity(blockSel.Position) is BlockEntityGenerator entity)
            {
                ...
                if (world.BlockAccessor.GetBlock(blockPos1) is IMechanicalPowerBlock block
        ...
        public override void OnNeighbourBlockChange(IWorldAccessor world, BlockPos pos, BlockPos neibpos)
        {
            ...
            if (world.BlockAccessor.GetBlockEntity(pos) is BlockEntityGenerator entity
                && FacingHelper.Faces(entity.Facing).First() is { } blockFacing)
                if (!world.BlockAccessor.GetBlock(pos.AddCopy(blockFacing)).SideSolid[blockFacing.Opposite.Index])
            ...
            Vintagestory.API.Common.Block[] chunkExtBlocks, int extIndex3d)
        {
            ...
            if (api is ICoreClientAPI clientApi)
                if (api.World.BlockAccessor.GetBlockEntity(pos) is BlockEntityGenerator entity &&
